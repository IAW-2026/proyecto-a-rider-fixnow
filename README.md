...
7. **Flujo Programado (Multa):** Cree un turno, asigne un profesional y luego cancélelo manualmente. El sistema detectará que ya había un trabajador comprometido y le exigirá el pago de una multa mediante un modal. Si presiona "Cancelar", verá cómo la tarjeta del trabajo ahora se actualiza y menciona que hay una multa pendiente, junto al boton para abonarla. Para corroborar que tampoco podrá solicitar otro servicio si no abona dicha multa, vuelva a Inicio. Verá que ahora aparece una tarjeta con el trabajo recien cancelado y con "Multa pendiente". Si selecciona dicho boton, se lo redirige a la pestaña de Trabajo Activo con los datos del trabajo cancelado y el boton "Abonar multa". Luego, la simulación de pago es la misma que ya mencionamos.
